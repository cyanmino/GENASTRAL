...
export const SolarAnnualCalendar = ({ year, periods, onAddPeriod, onRemovePeriod }: SolarAnnualCalendarProps) => {
  ...
  const handleAdd = () => {
    ...
    onAddPeriod({ year, start: startIso, end: endIso, colorKey, note: note.trim() || undefined });
    setStart(startIso);
    setEnd(endIso);
    setNote("");
  };
    ...
    <div className="panel" style={{ flex: 1, display: "flex", flexDirection: "column", gap: "0.75rem", minHeight: 0 }}>
      <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end" }}>
          <button type="button" onClick={handleAdd}>Agregar periodo</button>
        </div>
      </div>
